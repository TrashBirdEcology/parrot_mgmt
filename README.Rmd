...
5. Edit the terms (keywords, search phrases). Edit the code chunk containing terms
- See section: `Set a term or loop through terms`  
- `terms = [...]`
...
tmp <- parrot_xdd %>% distinct(coveryear, pubname, publisher, title, authors, searchterm)   
...
When we remove the redundancies in the records returned using the abovementioned searches,`r parrot_xdd %>% dplyr::select(-filename, -highlight, -searchterm) %>% n_distinct() + parrot_xdd %>% dplyr::select(-filename, -highlight, -searchterm) %>% filter(is.na(doi))%>% distinct()  %>% nrow() ` remain. 
...
tmp <- parrot_xdd %>% distinct(coveryear, pubname, publisher, title, authors, searchterm)%>% group_by(pubname) %>% 
         summarise(n = n()) 
...
dir.create("out")
write.csv(parrot_xdd %>% distinct("pubname","publisher","X_gddid","title","doi","coverDate","URL","authors"   , "highlight", "filename", "coveryear" ), "out/unique_results.csv")
```
